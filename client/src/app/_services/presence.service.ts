...
export class PresenceService {
  ...
  createHubConnection(user: User) {
    ...
    this.hubConnection.on('UserIsOnline', username => {
      this.usersOnline.pipe(take(1)).subscribe(usernames => {
        this.usersOnlineSource.next([...usernames, username]);
      });
    });

    this.hubConnection.on('UserIsOffline', username => {
      this.usersOnline.pipe(take(1)).subscribe(usernames => {
        this.usersOnlineSource.next([...usernames.filter(x => x !== username)]);
      });
    });
